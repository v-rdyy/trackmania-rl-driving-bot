    ...
    for episode_records in grouped.values():
        ...
        for record in active:
            if bool(record.get("steering_slope_reversal", False)):
                ...
                if excess:
                    penalized_event_count += 1
                    frequency_costs.append(0.05 * excess)

    expected = sum(
        int(episode["trajectory"]["precision"]["steering"]["significant_direction_reversals"])
        for episode in summary["episodes_detail"]
    )
    if len(events) != expected or logged_reversal_count != expected:
        raise ValueError(
            "V6 reversal instrumentation does not reproduce evaluator metrics: "
            f"detected={len(events)} logged={logged_reversal_count} expected={expected}"
        )
    ...
    metrics.update(
        {
            ...
            "penalized_event_count": penalized_event_count,
            "total_frequency_cost": sum(frequency_costs),
            "mean_frequency_cost_per_episode": (
                sum(frequency_costs) / len(grouped)
            ),
            "maximum_single_event_cost": max(frequency_costs, default=0.0),
